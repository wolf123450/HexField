...
export interface Attachment {
  ...
  mimeType:        string
  url?:            string
  inlineData?:     string        // base64 for Phase 1 ≤40KB inline
...
//
// Encoded as URL-safe base64 JSON in the hexfield://join/<b64> link.
...
/**
 * JoinCapsule — generated by a user who wants to join a server.
 * Shared (out of band) with an admin who scans/clicks the link and
 * manually approves the join without needing an invite code.
 * Encoded as base64-URL in a `hexfield://approve/{base64}` deep link.
 */
export interface JoinCapsule {
  v:             1
  userId:        string
  displayName:   string
  publicSignKey: string
  publicDHKey:   string
  endpoints:     PeerEndpoint[]  // so admin can connect for manifest delivery
  serverId:      string
  serverName:    string
}
