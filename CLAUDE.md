...
| Named dev instances (`dev:alice`, `dev:bob`) pop up a window and instantly close — **three separate causes**: (1) `tauri-plugin-deep-link` single-instance named pipe; (2) WebView2 UserDataDir lock; (3) WebView2 can't find runtime if LOCALAPPDATA is overridden | (1) Skip `tauri_plugin_deep_link::init()` when `HEXFIELD_MULTI_INSTANCE=1`. (2) Window is now created programmatically in `setup()` via `WebviewWindowBuilder` with `.data_directory()` set to `HEXFIELD_DATA_DIR` — isolates WebView2's EBWebView folder per instance. (3) Do NOT override LOCALAPPDATA — WebView2 needs it to find its runtime at `%LOCALAPPDATA%\Microsoft\EdgeWebView\`. The `tauri.conf.json` `windows` array is empty; all window config lives in `lib.rs`. |
...
| `migrate_data_urls_to_files` queries `avatar_data_url`/`banner_data_url` columns that migration 012 drops — fails with "no such column" on fresh DBs | Both sections now use `if let Ok(mut stmt) = conn.prepare(...)` so they skip gracefully when the columns don't exist |
| `canvas.toDataURL()` throws `SecurityError` when source image is an `asset://` URL (cross-origin canvas taint) | Added `img.crossOrigin = 'anonymous'` before `img.src = src` in `AvatarImage.vue`; wrapped `toDataURL()` in `try/catch` to fall back to `null` (animated GIF display) |
| `JoinView.vue` always calls `connectToPeer(invite.userId)` even when mDNS has already connected that peer — `create_offer` replaces the mDNS peer entry, the old PC fires `webrtc_disconnected`, and `destroyPeer` removes the new entry | (1) `JoinView.vue` now skips `connectToPeer` if already in `connectedPeers`. (2) `webrtc_manager.rs` `PeerEntry` has a `being_replaced: Arc<AtomicBool>` flag; `create_offer`/`handle_offer` set it on any old entry before replacing, so its `on_peer_connection_state_change(Closed)` callback skips emitting `webrtc_disconnected` |
| `serde_json` without `preserve_order` uses `BTreeMap` (alphabetical keys). Signal payloads with nested objects (e.g. `signal_ice` → `candidate: { candidate, sdpMid, sdpMLineIndex }`) had nested keys reordered in transit through the Rust WS/LAN relay. Fix: added `deepSortObj()` helper that recursively sorts ALL object keys before canonicalization. Both `signJson` and `verifyJsonSignature` now use `canonicalJson()` which deep-sorts. |
