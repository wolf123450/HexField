...
- [ ] Custom user avatar upload (static + animated GIF) in own-profile view in `UserProfileModal` (spec §16a)
- [ ] `<AvatarImage>` component — replaces initials circles everywhere; GIF frozen by default, animates on hover (spec §16a)
- [ ] Broadcast avatar + profile changes to peers via `profile_update` P2P mutation (spec §16a)
- [ ] Server avatar upload (admin only) in server settings; shown in `ServerRail` replacing initials circle (spec §16b)
- [ ] Broadcast server avatar via `server_update` mutation to joining peers (spec §16b)
...
- [ ] `<StatusBadge>` component — per-status SVG shape (circle/crescent/minus/hollow) + color for colorblind accessibility; replaces plain CSS dot everywhere (spec §17)
